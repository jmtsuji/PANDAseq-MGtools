...
# FastQ length analyzer
# Description: input a FastQ file, and this script outputs the lengths of the sequences, and, if desired, summary stats (NOT yet implemented) and a plot of their distribution.
# Created by Jackson Tsuji on April 17th, 2016, from fasta_length_analyze_v1.0.9.R
...
if (length(inputArgs) < 2) {
  stop("FastQ Length Analyzer, version 1.0.9, by Jackson Tsuji (Neufeld Research Group; jackson.tsuji@uwaterloo.ca) \n
        Description: outputs a tabular and graphical summary of the lengths of sequences in a FastQ file. Requires R and the ggplot2 R library.
        This script needs at least two input arguments. Must provide (in this order): \n
       1. The input filename (should be a FastQ file with multiple sequences). \n
...
lengths <- system(paste("awk '{ if (NR%4 == 2) { print length($0) } }' ", fastq_filename, sep = ""), intern = TRUE)
